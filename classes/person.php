...
$AllAdv="SELECT * from advertisment";
$ResAdv=$DB->selectDB($AllAdv);

$add_id=$this->CheckAddress($add);

$query="INSERT INTO customer(fullname,Email,Password,question_num,Answer,add_id,ssn,user_type_id) VALUES ('$name','$mail','$pass','$quest','$Ans','$add_id','$ssn',1) ";
$resAdd= $DB->addDB($query);
$lastCustID=mysqli_insert_id($DB->getCon());

$query2="INSERT INTO obbservers(cust_id,subscribe) VALUES ('$lastCustID',1) ";
$resAdd2= $DB->addDB($query2);

while ($row=mysqli_fetch_assoc($ResAdv)) {
$advID=$row['id'];
$sqlQue="INSERT INTO read_advert(adv_id,cust_id,read_status) VALUES ('$advID','$lastCustID',0) ";
$resQue= $DB->addDB($sqlQue);
}
